def extract_website_datas(website_container):
    ...
    header = website_container.find_element_by_tag_name('header').text
    ...
    website_info[header] = website_links
    return website_info
...
def extract_phone_number(phone_container):
    ...
    phone_info={}
    #get header of the container
    header = phone_container.find_element_by_tag_name('header').text
    #get all website links 
    phone_number = phone_container.find_element_by_xpath(
        '//span[@class="t-14 t-black t-normal"]').text
    phone_info[header] = phone_number
    return phone_info
...
def extract_address_datas(address_container):
    ...
    address_info={}
    #get header of the container
    header = address_container.find_element_by_tag_name('header').text
    #get all website links 
    ...
    address_info[header] = address + " : " + address_google_maps_link
    return address_info
...
def extract_email_datas(email_container):
    ...
    email_info={}
    #get header of the container
    header = email_container.find_element_by_tag_name('header').text
    #get email
    email = email_container.find_element_by_tag_name('a').get_attribute('href')
    email_info[header] = email
    return email_info